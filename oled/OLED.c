...
/**
 * @brief 通过HAL库控制SCL引脚的电平
 * @param BitValue 电平值，0为低电平，1为高电平
 * @note 该函数用于控制OLED显示屏通信中的SCL线
 */

void OLED_W_SCL(uint8_t BitValue) {
    /* 根据BitValue的值，将SCL置高电平或者低电平 */
    if (BitValue == 0) {
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8, GPIO_PIN_RESET); // SCL引脚置低
    } else {
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8, GPIO_PIN_SET); // SCL引脚置高
    }

    /* 如果单片机速度过快，可以在此添加适量延时，以避免超出I2C通信的最大速度 */
    // HAL_Delay(1);  // 可根据需要添加适当的延时
}
...
void OLED_W_SDA(uint8_t BitValue) {
    ...
    if (BitValue == 0) {
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_9, GPIO_PIN_RESET); // SDA引脚置低
    } else {
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_9, GPIO_PIN_SET); // SDA引脚置高
    }
